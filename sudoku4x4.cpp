...
void displayInstructions()
{
    ...
    << "The rules of the game are simple: each of the 4 blocks has to \n"
    << "contain  all the numbers 1-4 within its squares. Each number can only \n"
    << "appear once in a row, column or box. To make a move, enter the row \n"
    << "letter and column number corresponding to the square (Ex. A3) and then \n"
...
{
    return ( toupper(rowLetter) - 'A') * 4 + (colNumber - '0');
}
...
void initializeTheBoard( int &numberOfSquaresFilledIn, char board[])
{
    ...
    board[indexOfRowAndCol('A','3')] = '2';

    board[indexOfRowAndCol('B','2')] = '4';

    board[indexOfRowAndCol('C','2')] = '3'; board[indexOfRowAndCol('C','3')] = '1';



    // set the number of squares filled in so far, to use in checking for end of game
    numberOfSquaresFilledIn = 4;
}
...
bool userInputIsInvalid( char positionLetter, char positionNumber, char positionValue)
{
    if ( ((positionLetter < 'A') || (positionLetter > 'F')) ||
        ((positionNumber < '1') || (positionNumber > '4')) ||
        ((positionValue < '0') || (positionValue > '4'))
        ) {
...
{
    // check row
    int rowStart = (positionLetter - 'A')*4 + 1;    // get starting index for row
    ...
    for (int i=colStart; i<= colStart+12; i+=4) {
        if( positionValue == board[ i]) {
    ...
    int rowChange = (positionLetter - 'A')%2 * 4;
    ...
    if ( (positionValue == board[index])  || (positionValue == board[index+1]) ||
        (positionValue == board[index+4]) || (positionValue == board[index+5]))
    {
